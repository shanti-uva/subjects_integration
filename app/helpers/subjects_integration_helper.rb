...
module SubjectsIntegrationHelper
  ...
  def category_fields( options = {} )
    ...
    if selected_categories.empty?
      ...
      selected_categories.each do |c|
        result << "<span id=\"#{unique_id}_bin_item_#{c.id}\" class=\"tree-names\" style=\"line-height:19px;white-space:nowrap;padding:2px 3px 2px 2px; color:#404040; background-color:#f1f1f1; border:1pt #ccc solid;margin-right:3px;font-size:7pt\"><a href=\"#\" class=\"tree-remove\">#{image_tag('subjects_integration/delete.png', :size => '16x16', :border => '0', :alt => 'x', :style => 'display:inline;position:relative;top:4px;left:-2px')}</a>#{c.header}</span>\n"
  ...
  def category_selector_includes_old
    [javascript_include_tag('thickbox-compressed', 'subjects_integration/category_selector'), stylesheet_link_tag('thickbox', 'subjects_integration/category_selector')].join("\n").html_safe
  end
  
  def category_searcher(includes = true, options = {})
    return_str = ''
    if includes
      return_str << stylesheet_link_tag('subjects_integration/application')
      return_str << javascript_include_tag('subjects_integration/application')
    end
    selected_object = ""
      ...
      <script type=\"text/javascript\">
        ...
        jQuery(document).ready(function(){
          #{js_variable_name} = new ModelSearcher();
        	#{js_variable_name}.init('#{div_id}', '#{SubjectsIntegration::Feature.find(category_id).get_url(:list_with_places, :format => 'json')}', '#{SubjectsIntegration::Feature.find(category_id).get_url(:all_with_places, :format => 'json')}', {
  ...
  def category_selector(unique_id, selected_categories, options)
    ...
    return_str = options[:include_js] ? javascript_include_tag('subjects_integration/application') : ''
    return_str << stylesheet_link_tag('subjects_integration/application') if options[:include_styles]
    
    div_id = "#{unique_id}_tmb_category_selector"
    selected_objects = selected_categories.collect{|c| "{id: '#{c.id}', name: '#{escape_javascript(c.header)}'}" }
      ...
      <script type=\"text/javascript\">
        var #{js_variable_name} = new ModelSearcher(),
            #{js_variable_name}_tmb_options = {
              ...
              all_url_topic: '#{SubjectsIntegration::Feature.get_url_template(:all, :format => 'json')}'
          	};
  ...
  def topic_filter( options = {} )
    ...
    cats = root.nil? ? SubjectsIntegration::Feature.all : root.children
  ...
  def category_selector_old(main_category, instance_variable_name, field_name, includes = true)
    ...
    if selected_category.nil?
      ...
      return_str << selected_category.header
      options[:selected_category_id] = selected_category.id
    end
    category_url = category_children_path(main_category, options)
    return_str << "</span>\n("
    return_str << link_to("select #{h(main_category.header)}", category_url, :class => 'thickbox', :id => tag_prefix, :title => '') +
                  ")\n" +
  ...
  def loading_kmaps_animation_script(id)
    "$(\'##{id}_div\').css(\'background\', \'url(#{image_url('subjects_integration/loadingAnimation2.gif')}) no-repeat center right\')"
  end
end
