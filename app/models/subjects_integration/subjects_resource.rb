module SubjectsIntegration
  class SubjectsResource < ActiveResource::Base
    ...
    when InterfaceUtils::Server::DEVELOPMENT
      self.site = 'http://dev-subjects.kmaps.virginia.edu' #"dev-subjects.#{InterfaceUtils::Server.get_domain}"
    when InterfaceUtils::Server::STAGING
      self.site = 'http://staging-subjects.kmaps.virginia.edu' #"staging-subjects.#{InterfaceUtils::Server.get_domain}"
    when InterfaceUtils::Server::PRODUCTION
      self.site = 'http://subjects.kmaps.virginia.edu' #"subjects.#{InterfaceUtils::Server.get_domain}"
